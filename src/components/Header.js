                ...
                <div className="inner">
                    <div className="site-header-inside">
                        <div className="site-branding">
                            ...
                            {((_.get(page, 'sys.contentType.sys.id') === 'landing') || (_.get(page, 'sys.contentType.sys.id') === 'blog')) ?
                                <h1 className="site-title"><Link to={safePrefix('/')}>{_.get(config, 'header.title.title')}</Link></h1>
                                :
                                <p className="site-title"><Link to={safePrefix('/')}>{_.get(config, 'header.title.title')}</Link></p>
                            }
                        </div>
                                    ...
                                    <div className="site-nav-inside">
                                        ...
                                        <ul className="menu">
                                            {_.map(_.get(config, 'header.nav_links'), (action, actionIdx) => (
                                                <li key={actionIdx} className={'menu-item' + ((_.trim(_.get(this.props, 'path'), '/') === _.trim(_.get(action, 'url.url'), '/')) ? ' current-menu-item' : '') + (_.get(action, 'primary') ? ' menu-button' : '')}>
